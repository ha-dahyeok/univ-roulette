...
GATES = [
    {"name": "정문", "lat": 37.557604, "lng": 127.042020},
    {"name": "건축관 입구", "lat": 37.554054, "lng": 127.044191},
    {"name": "학생생활관 입구", "lat": 37.559538, "lng": 127.046159}
]

CSV_PATH = 'c:/Ha_dahyeok/2026_NEXT_CONTEST/hanyang_restaurants.csv'
SEARCH_RADIUS = 300
...
def determine_price(name, category):
    ...
    return '1' # 왕십리/한대앞 상권 특성상 알 수 없는 작은 밥집은 1단계(1.2만 이하) 비율이 높음
        ...
        for page in range(1, 46):
            ...
            try:
                ...
                for p in places:
                    ...
                    clean_name = name.replace(" 한양대점", "").replace(" 왕십리점", "").replace(" 본점", "").replace(" 한양대역점", "")
                    ...
                    row = ["한양대학교 서울캠퍼스", name, short_cat, gate['name'], price, url_link]
                    new_rows.append(row)
                    existing.add(name)
