...
GATES = [
    {"name": "정문", "lat": 37.552665, "lng": 126.924263},
    {"name": "후문(과학관)", "lat": 37.551546, "lng": 126.927444},
    {"name": "남문", "lat": 37.548697, "lng": 126.925281}
]

CSV_PATH = 'c:/Ha_dahyeok/2026_NEXT_CONTEST/hongik_restaurants.csv'
SEARCH_RADIUS = 300
...
def determine_price(name, category):
    ...
    return '2' # Default to 2 for unknown ones in 홍대상권 to ensure no blanks
        ...
        for page in range(1, 46):
            ...
            try:
                ...
                for p in places:
                    ...
                    clean_name = name.replace(" 홍대점", "").replace(" 홍대역점", "").replace(" 본점", "").replace(" 홍대입구역점", "")
                    ...
                    row = ["홍익대학교 서울캠퍼스", name, short_cat, gate['name'], price, url_link]
                    new_rows.append(row)
                    existing.add(name)
