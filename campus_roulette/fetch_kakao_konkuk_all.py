...
GATES = [
    {"name": "정문(상허문)", "lat": 37.539421, "lng": 127.072432},
    {"name": "건국문(후문)", "lat": 37.545224, "lng": 127.076576},
    {"name": "생명과학관부속동 입구(서문)", "lat": 37.541933, "lng": 127.071622}
]

CSV_PATH = 'c:/Ha_dahyeok/2026_NEXT_CONTEST/konkuk_restaurants.csv'
SEARCH_RADIUS = 300
...
def determine_price(name, category):
    ...
    return '2' # Default to 2 for unknown ones in 건대상권 to ensure no blanks
        ...
        for page in range(1, 46):
            ...
            try:
                ...
                for p in places:
                    ...
                    clean_name = name.replace(" 건대점", "").replace(" 건대역점", "").replace(" 본점", "").replace(" 건대입구역점", "")
                    ...
                    row = ["건국대학교 서울캠퍼스", name, short_cat, gate['name'], price, url_link]
                    new_rows.append(row)
                    existing.add(name)
